...
(function (root, factory) {
  ...
  // `extent` (optional) is the box a gradient spans — pass the union of a
  // group of connected same-fill regions and each one samples its slice of
  // one continuous ramp. Defaults to the region itself (self-contained).
  // Patterns ignore it (they anchor to the global lattice already).
  function paintRegionFill(ctx, region, fill, extent) {
    ...
    if (f.type === 'linear') {
      // Gradient spans `ext`; this region shows its slice (clipped above).
      const cx = ext.x + ext.w / 2, cy = ext.y + ext.h / 2;
      const dx = Math.cos(angle) * ext.w / 2, dy = Math.sin(angle) * ext.h / 2;
      const g = ctx.createLinearGradient(cx - dx, cy - dy, cx + dx, cy + dy);
      g.addColorStop(0, c1); g.addColorStop(1, c2);
      ctx.fillStyle = g; ctx.fillRect(x, y, w, h);
    } else if (f.type === 'radial') {
      const cx = ext.x + ext.w / 2, cy = ext.y + ext.h / 2;
      const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, Math.max(ext.w, ext.h) / 2);
      g.addColorStop(0, c1); g.addColorStop(1, c2);
      ctx.fillStyle = g; ctx.fillRect(x, y, w, h);
  ...
  // Stable identity for grouping regions that share a fill (used to decide
  // which regions a gradient should span together).
  function fillKey(fill) {
    const f = normalizeFill(fill);
    return [f.type, f.c1 || '', f.c2 || '', f.angle || 0, f.scale || 0].join('|');
  }

  return { TYPES, normalizeFill, paintRegionFill, fillToCSS, fillKey, mix };
}));
